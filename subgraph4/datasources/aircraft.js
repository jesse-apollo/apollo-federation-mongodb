...
class AircraftAPI extends DataSource {
    constructor(database) {
        super();
        this.database = database
        this.aircraft = this.database.collection("Aircraft");
    ...
    async getAircraftByID(id, info) {

        const query = { _id: new ObjectId(id) };
        const result = await this.aircraft.findOne(query);

        if (result == null) {
            return false;
        }

        return this.reMap(result, info.schema.getType('Aircraft'));
    }
  
    async getAircraftByCode(code, info) {
    ...
    async getAircraft(manufacturer, info) {
        ...
        const cursor = this.aircraft.find(query, options);
